...
\item （高优先级）增加博士论文国家图书馆格式封面，此封面仅当~phd~作为模板参数时才会生成。

\item （低优先级）增加本科生论文格式，当~bachelor~作为模板参数时生成本科生毕业论文格式。
...
~amssymb、~bm~、~geometry~、~enumitem~和~hyperref。
目前大多数的~\TeX{}~系统中都包含有这些宏包。
即使您系统中的宏包不全，您也可以到~CTAN~下载到这些宏包。
...
如果您使用~\texttt{BibTeX}~，那么在~Windows~下您可以直接双击~\texttt{make.bat}~
或者在命令行当前目录下执行~\texttt{make}~来生成~pdf~文件。

如果您不使用~\texttt{BibTeX}，那么您需要编辑~\texttt{make.bat}~内容去掉~\texttt{BibTeX}~
命令。
