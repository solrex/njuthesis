...
\documentclass[dvipdfm, oneside, phd]{NJUthesis}
% 可选参数：
...
%
% 国家图书馆封面内容——仅博士需要填写并保证模板参数选择了 phd
%
\classification{}
\confidential{}
\UDC{}
...
\texttt{NJUthesis}~宏包在~\texttt{CASthesis}~宏包的基础上根据南京大学对学位
论文的规定修改而来。宏包的另一个目的是简化学位论文的撰写，使得论文作者可以将
精力集中到论文的内容上而不是浪费在版面设置上。同时宏包在符合学位论文撰写要求
的基础上尽可能地进行美化，其中还参考了出版界的一些排版规范。

本模板目前有两个选项：~\texttt{phd}~和~\texttt{master}~。当选择~
\texttt{phd}~ 时，模板会自动生成国家图书馆格式的封面，但不包括书脊，书脊需要
单独制作。
...
\texttt{NJUthesis}~宏包的最新版本可以从~
\url{http://share.solrex.org/njuthesis/}~网站下载。

\texttt{NJUthesis}~宏包主要包含两个文件：~\texttt{NJUthesis.cls}~和~
\texttt{NJUthesis.cfg}，还有一个额外的~\texttt{BibTeX}~格式控制的~
\texttt{NJUthesis.bst}，不过您完全可以使用自己的~\texttt{.bst}~文件或者根本
不使用~\texttt{BibTeX}。
...
如果您使用~\texttt{BibTeX}~，那么在~Windows~下您可以直接双击~
\texttt{make.bat}~ 或者在命令行当前目录下执行~\texttt{make}~来生成~pdf~文件
。

如果您不使用~\texttt{BibTeX}，那么您需要编辑~\texttt{make.bat}~内容去掉~
\texttt{BibTeX}~ 命令。
...
本章主要介绍参考文献的格式，在~\texttt{NJUthesis}~中控制参考文献的主要有两个
地方：
